// Stockyard Paddock — Uptime monitor.
// Monitor endpoints, track uptime, share a public status page. Self-hosted.
...
import (
	...
	"github.com/stockyard-dev/stockyard-paddock/internal/license"
...
func main() {
	...
	// License validation
	licenseKey := os.Getenv("PADDOCK_LICENSE_KEY")
	licInfo, licErr := license.Validate(licenseKey, "paddock")
	...
	if licInfo != nil && licInfo.IsPro() {
		...
		log.Printf("  License:   Free tier (set PADDOCK_LICENSE_KEY to unlock Pro)")
	}
	...
	log.Printf("")
	log.Printf("  Stockyard Paddock %s", version)
	log.Printf("  API:            http://localhost:%d/api/monitors", port)
	log.Printf("  Status page:    http://localhost:%d/status", port)
	...
	go func() {
		for {
			...
			if err != nil {
				...
				log.Printf("[cleanup] deleted %d checks older than %d days", n, retentionDays)
			}
		}
	}()
